...
public class CommandParser {
    ...
    public void addParsingRule(String commandName, List<ArgumentType> fieldsType) throws Exception
    {
        ...
        if (this.parsingRules.containsKey(commandName)) throw new Exception("Parsing rule for this command already exists");
    ...
    public Pair<String, List<String>> parse(String commandLine) throws Exception
    {
        ...
        for (int i=0; i < argumentTypes.size(); i++)
        {
            ...
            {
                case ALPHA: parsedArguments.add(parseAlpha(rawArguments.get(i).trim())); break;
                case DIGIT: parsedArguments.add(parseDigit(rawArguments.get(i).trim())); break;
                case DATE_TIME: parsedArguments.add(parseDateTime(rawArguments.get(i).trim())); break;
                case DATE: parsedArguments.add(parseDate(rawArguments.get(i).trim())); break;
            }
        }
